...
initial_ring_width = ring_width;
iteration = 1;
while size(inner_points,1) > 8
  ...
  if ring_width < 0.5*initial_ring_width && mod(size(inner_points,1) - 1, 2) == 0
    iteration = 1;
...
c(end+1,:) = [ num_points , num_points - 4 , num_points - 2 ];
c(end+1,:) = [ num_points , num_points - 6 , num_points - 4 ];
